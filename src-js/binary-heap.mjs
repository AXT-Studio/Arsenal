...
class BinaryHeap {
     ...
     * @param {(a: unknown, b: unknown) => number} compareFn - 比較関数(aの優先度が高いなら負、bの優先度が高いなら正、等しいなら0)
     * @param {unknown[]} [initialValues] - 初期値の配列 (省略可)
     * @constructor
     */
    constructor(compareFn = DEFAULT_COMPARE_FN, initialValues = []) {
        ...
        for (let i = 1; i <= this.#elements[0]; i++) {
            ...
            if (indexSet) {
                ...
                this.#indexMap.set(value, new Set([i]));
            }
        }
    }
    /**
     ...
     * @param {boolean} [editIndexMap=true] - indexMapも更新するかどうか (falseにするのはheapify時くらい)
     * @return {number} 最終的な配置先インデックス
     */
    #downHeap(startIndex, editIndexMap = true) {
    ...
    /**
     * 指定要素をヒープから削除する
     * @param {unknown} value - 削除する要素
     * @returns {boolean} 指定要素が存在して削除された場合は`true`、存在しなかった場合は`false`
     */
    remove(value) {
        const indexSet = this.#indexMap.get(value);
    ...
    /**
     * ヒープ内の要素の値を更新する
     * @param {unknown} oldValue - 更新前の値
     * @param {unknown} newValue - 更新後の値
     * @returns {boolean} 更新に成功した場合は`true`、`oldValue`が見つからなかった場合は`false`
     */
