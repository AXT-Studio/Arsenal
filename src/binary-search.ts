// ================================================================================================
// entrypoint: binary-search
// ソート済み配列に対して二分探索を行うメソッド群を提供します。
// C++のSTLにある同名の関数にインスパイアされています。
// ================================================================================================

// ================================================================
// 関数本体
// ================================================================

/** Array#sort()でcompareFnを指定しなかったときのデフォルトの挙動と同じ挙動を示す比較関数 */
const DEFAULT_COMPARE_FN = (a: unknown, b: unknown): number => {
    const [A, B] = [String(a), String(b)];
    return (A < B) ? -1 : (A > B) ? 1 : 0;
};

/**
 * 配列`array`に`target`と等しい値が存在するかどうかを、二分探索を用いて判定する。
 * @param array - ソート済み配列
 * @param target - 探索対象の値
 * @param compareFn - 比較関数 (省略時は文字列として比較)
 * @returns `target`と等しい値が存在すれば`true`、そうでなければ`false`
 */
const binary_search = <T>(
    array: readonly T[],
    target: T,
    compareFn: (a: T, b: T) => number = DEFAULT_COMPARE_FN,
): boolean => {
    let low = 0;
    let high = array.length - 1;

    while (low <= high) {
        const mid = Math.floor((low + high) / 2);
        const cmp = compareFn(array[mid], target);

        if (cmp === 0) return true;
        if (cmp < 0) low = mid + 1;
        else high = mid - 1;
    }

    return false;
};

/**
 * 配列`array`の中で、`target`以上と判定される最初の要素のインデックスを返す。
 * (`array`内に`target`以上の要素が存在しない場合は`array.length`を返す。)
 * @param array - ソート済み配列
 * @param target - 探索対象の値
 * @param compareFn - 比較関数 (省略時は文字列として比較)
 * @returns `target`以上と判定される最初の要素のインデックス
 */
const lower_bound = <T>(
    array: readonly T[],
    target: T,
    compareFn: (a: T, b: T) => number = DEFAULT_COMPARE_FN,
): number => {
    let low = 0;
    let high = array.length;

    while (low < high) {
        const mid = Math.floor((low + high) / 2);
        const cmp = compareFn(array[mid], target);

        if (cmp < 0) low = mid + 1;
        else high = mid;
    }

    return low;
};

/**
 * 配列`array`の中で、`target`より大きいと判定される最初の要素のインデックスを返す。
 * (`array`内に`target`より大きい要素が存在しない場合は`array.length`を返す。)
 * @param array - ソート済み配列
 * @param target - 探索対象の値
 * @param compareFn - 比較関数 (省略時は文字列として比較)
 * @returns `target`より大きいと判定される最初の要素のインデックス
 */
const upper_bound = <T>(
    array: readonly T[],
    target: T,
    compareFn: (a: T, b: T) => number = DEFAULT_COMPARE_FN,
): number => {
    let low = 0;
    let high = array.length;

    while (low < high) {
        const mid = Math.floor((low + high) / 2);
        const cmp = compareFn(array[mid], target);

        if (cmp <= 0) low = mid + 1;
        else high = mid;
    }

    return low;
};

// ================================================================
// エクスポート
// ================================================================

export { binary_search, lower_bound, upper_bound };
