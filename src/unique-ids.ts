...
/**
 * UUIDv7を生成します。
 * @param [timestamp] - UNIXタイムスタンプ (ミリ秒単位、48bit以内、省略時はDate.now())
 */
const getUUIDv7 = (timestamp: number = Date.now()): string => {
    ...
    const randomHexString = Array.from(
        randomUint8Array,
    ...
    return uuidBigInt.toString(16).padStart(32, "0").replace(
        /(.{8})(.{4})(.{4})(.{4})(.{12})/,
        "$1-$2-$3-$4-$5",
    ).toLowerCase();
};
