...
配列`array`の中で、`target`**以上**と判定される最初の要素のインデックスを返します。

### `lower_bound<T>(array: readonly T[], target: T, compareFn?: (a: T, b: T) => number): number`
...
- `number`
    - `array`内で`target`以上と判定される最初の要素のインデックスを返します。
